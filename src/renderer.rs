...
/// In this case, we're just using VK_LAYER_KHRONOS_validation, the 'default' layers.
...
/// Load Vulkan Commands in the form of an instance and configure Validation Layers if enabled.
unsafe fn create_instance(window: &Window, entry: &Entry, data: &mut AppData) -> Result<Instance> {
    // == Application Info == //

    // Here you specify information about your application to Vulkan
    ...
    // enumerate and collect names of available validation layers.
    ...
    // if requested validation layers are not available, throw an error.
    if VALIDATION_ENABLED && !available_layers.contains(&VALIDATION_LAYER) {
        return Err(anyhow!("Validation layer requested but not supported."));
    }

    // collect validation layers into a vector.
    ...
    // Extensions are extra features that vulkan provides but may not be supported on every platform.
    // Here, we get the extensions required by the window API and specify the extensions we want.
    // (in this case, we arent specifying any extensions)
    ...
    // If compiling for MacOS, enabled required extensions.
    ...
    // Configure debug callback (located right under this function) that will catch the
    // incoming validation layer messages so we can handle them as we see fit.
    let mut debug_info = vk::DebugUtilsMessengerCreateInfoEXT::builder()
        .message_severity(vk::DebugUtilsMessageSeverityFlagsEXT::all())
        .message_type(vk::DebugUtilsMessageTypeFlagsEXT::all())
        .user_callback(Some(debug_callback));

    // Add debug_info to infos' 'structure pointer chain',
    // extending the struct and associating it with debug_info. 
    if VALIDATION_ENABLED {
    ...
    // == Debug/Error Messenger == //

    // our 'messenger' will handle the callback we specified earlier. 
    if VALIDATION_ENABLED {
...
// Callback Function that allows us to specify custom handling for validation messages.
